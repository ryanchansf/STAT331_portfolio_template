...
# REVISION AFTER MIDTERM: since question numbers are all in the 900s, subtracting 900 from each in order to refer to them by their single/double digit identifier
...
# Source: Lab 5, Step 2
# REVISION: extend addresses ending with Dr or Ave to allow for better flexibility in filtering and consistent naming. Standardizing the street extensions makes this more robust and resists inconsistencies in filtering inputs for common mislabels like Dr or Drive.
...
person |>
  ...
  filter((address_street_name == "Northwestern Drive" & 
...
# Source: Lab 5, Step 5
# REVISION: converting date column into date type, then adding year and month columns for easy filtering
...
# Source: Lab 5, Step 6
# REVISION: reversing logic to perform right join where the focus is the hiring suspect, adding the interview information for Miranda Priestly if available
interview |>
  right_join(hiring_suspect,
...
# REVISION: using semi_join() to filter rows by a set of target years

target_years <- tibble(study_year = c(2008,
                                      2018))

ca_childcare |>
  semi_join(target_years,
            by = "study_year") |>
...
# Source: Lab 3, Question 12
# REVISION: using anti_join() to exclude a set of degrees instead of filtering by desired degrees

exclude_degrees = tibble(academic_degree = c("ma",
                                             "no_dgr"))

teacher_evals_clean |>
  anti_join(exclude_degrees,
            by = "academic_degree") |>
  group_by(teacher_id) |>
  filter(sex == "female") |>
...
# REVISION: remove tilting of axis labels
ggplot(data = surveys) +
